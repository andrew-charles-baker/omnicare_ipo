...
where $\Lambda(\cdot)$ is the logistic CDF. If higher fact-intensity reduces returns, we expect $\beta_\tau < 0$ (lower probability of exceeding any given threshold).
...
ggplot(dr_full, aes(x = threshold, y = coefficient)) +
  ...
  labs(
    ...
    y = "Coefficient on rf_fact_t07 (Logit)"
  ) +
  theme_minimal() +
...
This approach is related to quantile regression but estimates $P(Y > \tau | X)$ rather than conditional quantiles directly. The logit coefficients represent the change in log-odds of exceeding threshold $\tau$ for a one-unit increase in fact-intensity.
