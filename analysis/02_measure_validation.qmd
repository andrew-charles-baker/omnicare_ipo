...
## 7. Full CFM Decomposition with Multiple Covariates

Following CFM (2013), we now decompose the total change into three components:

1. **Structure Effect**: Changes in how the market prices IPOs (the conditional distribution $F_{Y|X}$)
2. **rf_fact Composition Effect**: Changes due to shifts in the fact-intensity distribution
3. **Other Composition Effect**: Changes due to shifts in other covariates (assets, proceeds, market conditions, industry)

The sequential decomposition path is:
$$F_{Y\langle 1|1 \rangle} - F_{Y\langle 0|0 \rangle} = \underbrace{[F_{Y\langle 1|1 \rangle} - F_{Y\langle 0|1 \rangle}]}_{\text{Structure}} + \underbrace{[F_{Y\langle 0|1 \rangle} - F_{Y\langle 0|\text{rf}_1, \text{other}_0 \rangle}]}_{\text{Other Composition}} + \underbrace{[F_{Y\langle 0|\text{rf}_1, \text{other}_0 \rangle} - F_{Y\langle 0|0 \rangle}]}_{\text{rf\_fact Composition}}$$

```{r}
#| label: cfm-full-functions
#| warning: false

# Function to estimate conditional distribution with multiple covariates
...
# Function to predict CDF given model and covariate matrix
...
# Main function for full CFM decomposition
cfm_full_decomposition <- function(data, outcome_var, main_covariate, other_covariates,
  ...
  # Prepare covariate data frames for counterfactuals
  X_before <- before_data[, all_covariates, drop = FALSE]
  X_after <- after_data[, all_covariates, drop = FALSE]

  # Create hybrid covariate set: rf_fact from post, other covariates from pre
  # We need to match the sample sizes, so we'll resample
  set.seed(42)
  ...
  # Verify: total = structure + other_comp + rf_comp
  decomp_check <- structure_effect + other_composition + rf_composition
...
# Note: excluding ff12 factor for numerical stability; can add back if needed

# Run decomposition with bootstrap
...
decomp_full <- full_decomp$decomposition

cat("=== FULL CFM DECOMPOSITION SUMMARY ===\n\n")
cat("Sample sizes:\n")
cat("  Pre-Omnicare:", full_decomp$sample_sizes$before, "\n")
cat("  Post-Omnicare:", full_decomp$sample_sizes$after, "\n\n")

cat("Average effects across distribution:\n")
cat(sprintf("  Total change:        %7.4f\n", mean(decomp_full$total_change)))
cat(sprintf("  Structure effect:    %7.4f\n", mean(decomp_full$structure_effect)))
cat(sprintf("  rf_fact composition: %7.4f\n", mean(decomp_full$rf_composition)))
cat(sprintf("  Other composition:   %7.4f\n", mean(decomp_full$other_composition)))
cat(sprintf("  Check (should = total): %.4f\n", mean(decomp_full$decomp_check)))
```

### Plot: Full Three-Way Decomposition

```{r}
#| label: cfm-full-plot
#| fig-cap: "Full CFM Decomposition: Structure, rf_fact, and Other Composition Effects"
#| fig-height: 7
#| warning: false

# Reshape for plotting
effect_long_full <- decomp_full |>
  ...
  dplyr::mutate(
    effect_label = dplyr::case_when(
      ...
      effect_type == "other_composition" ~ "Other Covariates Composition"
    ),
    effect_label = factor(effect_label, levels = c(
      "Total Change", "Structure Effect (Pricing)",
      "rf_fact Composition", "Other Covariates Composition"
    ))
  )

# Check if CIs available
has_ci <- "total_ci_lower" %in% names(decomp_full)

if (has_ci) {
  ggplot() +
    # Confidence ribbons (alpha increased for visibility)
    geom_ribbon(data = decomp_full,
                aes(x = y_threshold, ymin = total_ci_lower, ymax = total_ci_upper),
    ...
    # Effect lines
    geom_line(data = effect_long_full,
              aes(x = y_threshold, y = effect_value, color = effect_label),
              linewidth = 1.2) +
    geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
    scale_color_manual(values = c(
      ...
      "Other Covariates Composition" = "darkgreen"
    )) +
    labs(
      title = "Full CFM Decomposition: Three-Way Attribution",
      subtitle = "Total = Structure + rf_fact Composition + Other Composition",
      x = "First-Day Return Threshold",
      y = "Change in Cumulative Probability",
      color = "Effect Type",
      caption = "Shaded areas: 95% bootstrap confidence intervals\nOther covariates: log(assets), log(proceeds), roll_vwretd"
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(face = "bold", size = 14),
  ...
  ggplot(effect_long_full, aes(x = y_threshold, y = effect_value, color = effect_label)) +
    geom_line(linewidth = 1.2) +
    geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
    scale_color_manual(values = c(
      "Total Change" = "black",
...
### Interpretation of Full Decomposition

```{r}
#| label: cfm-full-interpretation

cat("=== FULL DECOMPOSITION INTERPRETATION ===\n\n")

avg_total <- mean(decomp_full$total_change)
avg_structure <- mean(decomp_full$structure_effect)
avg_rf <- mean(decomp_full$rf_composition)
avg_other <- mean(decomp_full$other_composition)

# Direction of total change
if (avg_total > 0) {
  cat("TOTAL CHANGE (", round(avg_total, 4), "): Post-Omnicare has MORE mass at lower returns.\n")
} else {
  cat("TOTAL CHANGE (", round(avg_total, 4), "): Post-Omnicare has LESS mass at lower returns.\n")
}

cat("\nDECOMPOSITION INTO THREE COMPONENTS:\n\n")

# Structure effect
cat("1. STRUCTURE EFFECT (", round(avg_structure, 4), "):\n")
cat("   How the market's PRICING FUNCTION changed, holding covariates fixed.\n")
if (avg_structure > 0) {
  cat("   → For same firm characteristics, post-Omnicare pricing yields LESS underpricing.\n")
} else {
  cat("   → For same firm characteristics, post-Omnicare pricing yields MORE underpricing.\n")
}

# rf_fact composition
cat("\n2. rf_fact COMPOSITION EFFECT (", round(avg_rf, 4), "):\n")
cat("   Effect of the shift in fact-intensity distribution, holding pricing & other X fixed.\n")

# Get actual rf_fact direction
rf_pre <- mean(df_complete[!df_complete$post_omnicare, "rf_fact_t07"], na.rm = TRUE)
rf_post <- mean(df_complete[df_complete$post_omnicare, "rf_fact_t07"], na.rm = TRUE)
rf_direction <- ifelse(rf_post > rf_pre, "increased (more facts)", "decreased (more opinions)")

cat(sprintf("   Fact: rf_fact %s (%.3f → %.3f)\n", rf_direction, rf_pre, rf_post))
if (avg_rf > 0) {
  cat("   → The rf_fact shift alone would have pushed toward LESS underpricing.\n")
} else {
  cat("   → The rf_fact shift alone would have pushed toward MORE underpricing.\n")
}

# Other composition
cat("\n3. OTHER COVARIATES COMPOSITION EFFECT (", round(avg_other, 4), "):\n")
cat("   Effect of shifts in assets, proceeds, and market conditions.\n")
if (avg_other > 0) {
  cat("   → Changes in other firm/market characteristics → LESS underpricing.\n")
} else {
  cat("   → Changes in other firm/market characteristics → MORE underpricing.\n")
}

# Relative contributions
cat("\n=== RELATIVE CONTRIBUTIONS ===\n")
total_abs <- abs(avg_structure) + abs(avg_rf) + abs(avg_other)
if (total_abs > 0) {
  cat(sprintf("  Structure effect:      %5.1f%%\n", 100 * abs(avg_structure) / total_abs))
  cat(sprintf("  rf_fact composition:   %5.1f%%\n", 100 * abs(avg_rf) / total_abs))
  cat(sprintf("  Other composition:     %5.1f%%\n", 100 * abs(avg_other) / total_abs))
}
```

---

## 8. Circuit Heterogeneity Analysis

Omnicare's impact varied by circuit. The **6th Circuit** (KY, MI, OH, TN) had previously held opinions actionable, so Omnicare *reduced* liability there. The **2nd and 9th Circuits** (CT, NY, VT + AK, AZ, CA, HI, ID, MT, NV, OR, WA) had previously treated opinions as non-actionable, so Omnicare *expanded* plaintiff avenues there.
...
sixth_circuit_states <- c("KY", "MI", "OH", "TN")

second_circuit_states <- c("CT", "NY", "VT")
ninth_circuit_states <- c("AK", "AZ", "CA", "HI", "ID", "MT", "NV", "OR", "WA")
second_ninth_states <- c(second_circuit_states, ninth_circuit_states)

# Create circuit indicator
df_complete$circuit <- case_when(
  df_complete$headquarter %in% sixth_circuit_states ~ "6th Circuit",
  df_complete$headquarter %in% second_ninth_states ~ "2nd/9th Circuit",
  TRUE ~ "Other"
)

# Summary
cat("=== CIRCUIT DISTRIBUTION ===\n\n")
circuit_summary <- df_complete %>%
...
print(circuit_summary %>% dplyr::select(circuit, period, n, mean_rf_fact, mean_return))
```

### Decomposition by Circuit

```{r}
#| label: circuit-decomposition
#| warning: false
#| fig-height: 10

# Function to run decomposition for a subset
run_circuit_decomposition <- function(data, circuit_name, n_boot = 200) {

  # Check sample sizes
  n_pre <- sum(!data$post_omnicare)
  n_post <- sum(data$post_omnicare)

  if (n_pre < 20 || n_post < 20) {
    return(NULL)
  }

  tryCatch({
    result <- bootstrap_cfm_full(
      data = data,
      outcome_var = "first_day_return",
      main_covariate = "rf_fact_t07",
      other_covariates = c("log_assets", "log_proceeds", "roll_vwretd"),
      group_var = "post_omnicare",
      n_bootstrap = n_boot
    )
    ...
    cat("Error for", circuit_name, ":", conditionMessage(e), "\n")
    return(NULL)
  })
}
...
decomp_6th <- run_circuit_decomposition(df_6th, "6th Circuit", n_boot = 200)

# Run for 2nd/9th Circuit
df_2nd9th <- df_complete[df_complete$circuit == "2nd/9th Circuit", ]
decomp_2nd9th <- run_circuit_decomposition(df_2nd9th, "2nd/9th Circuit", n_boot = 200)

# Report sample sizes
cat("\n=== SAMPLE SIZES FOR DECOMPOSITION ===\n")
if (!is.null(decomp_6th)) {
  cat("6th Circuit: Pre =", decomp_6th$sample_sizes$before,
      ", Post =", decomp_6th$sample_sizes$after, "\n")
}
if (!is.null(decomp_2nd9th)) {
  cat("2nd/9th Circuit: Pre =", decomp_2nd9th$sample_sizes$before,
      ", Post =", decomp_2nd9th$sample_sizes$after, "\n")
}
```

```{r}
#| label: circuit-plot
#| fig-cap: "CFM Decomposition by Circuit: 6th vs 2nd/9th"
#| fig-height: 10
#| warning: false
...
if (!is.null(decomp_6th)) {
  decomp_6th_df <- decomp_6th$decomposition
  decomp_6th_df$circuit <- "6th Circuit (Omnicare reduced liability)"

  effect_6th <- decomp_6th_df %>%
    dplyr::select(y_threshold, total_change, structure_effect, rf_composition, other_composition, circuit) %>%
    tidyr::pivot_longer(cols = c(total_change, structure_effect, rf_composition, other_composition),
  ...
  # Check for CIs
  has_ci_6th <- "total_ci_lower" %in% names(decomp_6th_df)

  p_6th <- ggplot(effect_6th, aes(x = y_threshold, y = effect_value, color = effect_label)) +
    geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5)

  if (has_ci_6th) {
    p_6th <- p_6th +
      geom_ribbon(data = decomp_6th_df,
                  aes(x = y_threshold, ymin = total_ci_lower, ymax = total_ci_upper),
      ...
      geom_ribbon(data = decomp_6th_df,
                  aes(x = y_threshold, ymin = other_ci_lower, ymax = other_ci_upper),
                  fill = "darkgreen", alpha = 0.15, inherit.aes = FALSE)
  }
  ...
  p_6th <- p_6th +
    ...
    scale_color_manual(values = c(
      "Total Change" = "black",
      "Structure Effect" = "red",
      "rf_fact Composition" = "blue",
      "Other Composition" = "darkgreen"
    )) +
    labs(
      title = "6th Circuit (KY, MI, OH, TN)",
      subtitle = "Omnicare REDUCED liability (overturned prior pro-plaintiff stance)",
      x = "First-Day Return Threshold",
      y = "Change in Cumulative Probability",
      color = "Effect"
    ) +
...
if (!is.null(decomp_2nd9th)) {
  decomp_2nd9th_df <- decomp_2nd9th$decomposition
  decomp_2nd9th_df$circuit <- "2nd/9th Circuit (Omnicare expanded liability)"

  effect_2nd9th <- decomp_2nd9th_df %>%
    dplyr::select(y_threshold, total_change, structure_effect, rf_composition, other_composition, circuit) %>%
    tidyr::pivot_longer(cols = c(total_change, structure_effect, rf_composition, other_composition),
                        names_to = "effect_type", values_to = "effect_value") %>%
    dplyr::mutate(
  ...
  has_ci_2nd9th <- "total_ci_lower" %in% names(decomp_2nd9th_df)

  p_2nd9th <- ggplot(effect_2nd9th, aes(x = y_threshold, y = effect_value, color = effect_label)) +
    geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5)

  if (has_ci_2nd9th) {
    p_2nd9th <- p_2nd9th +
      geom_ribbon(data = decomp_2nd9th_df,
                  aes(x = y_threshold, ymin = total_ci_lower, ymax = total_ci_upper),
                  fill = "black", alpha = 0.15, inherit.aes = FALSE) +
      geom_ribbon(data = decomp_2nd9th_df,
                  aes(x = y_threshold, ymin = structure_ci_lower, ymax = structure_ci_upper),
  ...
  p_2nd9th <- p_2nd9th +
    ...
    labs(
      title = "2nd/9th Circuit (NY, CA, etc.)",
      subtitle = "Omnicare EXPANDED liability (gave plaintiffs new avenues)",
      x = "First-Day Return Threshold",
      y = "Change in Cumulative Probability",
      color = "Effect"
    ) +
  ...
  plot_list[["2nd9th"]] <- p_2nd9th
}

# Combine plots
if (length(plot_list) == 2) {
  plot_list[["6th"]] / plot_list[["2nd9th"]] +
    plot_layout(guides = "collect") &
    theme(legend.position = "bottom")
} else if (length(plot_list) == 1) {
  plot_list[[1]]
} else {
  cat("Insufficient data for circuit-level decomposition\n")
}
```
...
```{r}
#| label: circuit-summary

cat("=== CIRCUIT COMPARISON SUMMARY ===\n\n")
...
# Interpretation
cat("INTERPRETATION:\n")
cat("If Omnicare's legal impact drives the results, we expect:\n")
cat("  - 6th Circuit: Reduced liability → possibly MORE opinions, different structure effect\n")
cat("  - 2nd/9th Circuit: Expanded liability → possibly FEWER opinions, different structure effect\n")
```
