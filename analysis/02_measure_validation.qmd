...
To visualize the partial relationship between fact-intensity and first-day returns after controlling for covariates, we use the Frisch-Waugh-Lovell approach:

1. Regress `first_day_return` on controls, obtain residuals
2. Regress `rf_fact_t07` on controls, obtain residuals
3. Plot binscatter of residualized Y vs. residualized X

This isolates the conditional relationship between the two variables, partialling out the effects of firm size, deal size, industry, and market conditions.
...
# Binscatter function for residualized data
binscatter_resid <- function(data, title = "") {
  x <- data$resid_x
  y <- data$resid_y
  n <- length(x)
  ...
  return(list(plot = p, fit = fit))
}

bs_ctrl <- binscatter_resid(df_complete,
                             title = "Binscatter with Controls (Full Sample)")
print(bs_ctrl$plot)

cat("\nPartial regression coefficient:", round(coef(bs_ctrl$fit)[2], 4), "\n")
cat("This should match the rf_fact_t07 coefficient from OLS with controls:",
